...
if __name__ == '__main__':
    # First process the triangle to be read-able
    cells = [i.split(" ") for i in TRIANGLE.split("\n")][1:-1]
    cells = [[int(j) for j in i] for i in cells][::-1]
    ...
    for i in range(1, len(cells)):
        for j in range(len(cells[i])):
            cells[i][j] += max(cells[i - 1][j], cells[i - 1][j + 1])
    print cells[-1][-1]
