...
if __name__ == '__main__':
    ...
    matrix = zip(*map(lambda x: map(int, x[:-1].split(",")), open(MATRIX_TXT, 'r').readlines()))
    ...
    distance = {node : '' if node[0] else matrix_value(node) for node in unvisited}
    while unvisited:
        # Get the node with the smallest value
        node = min((distance[node], node) for node in unvisited)[1]
        unvisited.remove(node)

        # Check each neighbour for a smaller path
        for increment in [(0, 1), (0, -1), (1, 0)]:
            other_node = tuple(sum(i) for i in zip(node, increment))
            if other_node in distance:
                path = distance[node] + matrix_value(other_node)
                distance[other_node] = min(path, distance[other_node])

    # Print out the minimum value from the left-most column
    print min(distance[(DIMENSIONS - 1, i)] for i in xrange(DIMENSIONS))
